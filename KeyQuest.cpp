/*
  ------------------------------------------------------------------------------
  KeyQuest V1.1 by Benjade https://github.com/Benjade
...
"</table>"
"<p class=\"small\">Generated by KeyQuest V1.1 by <a href=\"https://github.com/Benjade\">Benjade</a></p>"
"</div></body></html>";
                              ...
                              const std::string &totalCombosStr)
{
    ...
    {
        std::string title = " Statistics - KeyQuest V1.1 by Benjade ";
	...
	std::string endPrefix   = thr[tid].endHex  .substr(0, seqDigits);
		...
		if (g_fullRandomMode) {
    		...
    		padded = padHex(prefix + suffix, 64);
                ...
                ++localCount;
                // Calcul ECC + batch hash + comparison
                            ...
                            if(_mm_movemask_epi8(cmp) == 0xFFFF){
                                ...
                                if(!g_found.load()){
                                    ...
                                    sendMatchEmail(g_foundPriv,g_foundPub,g_foundWIF,
                                        targetAddress,tot,formatElapsedTime(dt),g_finalSpeed);
                                }
                                break;
                            }
                            ++batchCountLocal;
