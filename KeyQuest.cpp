/*
  ------------------------------------------------------------------------------
  KeyQuest V1.00 by Benjade https://github.com/Benjade
...
static void saveProgressToFile(const std::string &str){
...
"</table>"
"<p class=\"small\">Generated by KeyQuest V1.00 by <a href=\"https://github.com/Benjade\">Benjade</a></p>"
"</div></body></html>";
                              ...
                              const std::string &totalCombosStr)
{
    ...
    {
        std::string title = " Statistics - KeyQuest V1.00 by Benjade ";
...

    {
        ...
        tmp << COLOR_LABEL << "Target: "    << COLOR_VALUE << addr           << COLOR_RESET
            ...
            << COLOR_LABEL << "    Mode: "  << COLOR_VALUE << "Hybrid"       << COLOR_RESET
...

    {
        ...
        tmp << COLOR_LABEL << "Speed: "  << COLOR_VALUE
            ...
            << COLOR_LABEL << " / "      << COLOR_VALUE << totalCombosStr << " combos" << COLOR_RESET;
        auto content = padAnsi(tmp.str(), inner);
        os << COLOR_FRAME << "|" << COLOR_RESET
           << content
           << COLOR_FRAME << "|" << COLOR_RESET << "\n";
    ...
    {
        std::ostringstream tmp;
        tmp << COLOR_LABEL << "Progress: "<< COLOR_VALUE
            << std::fixed << std::setprecision(10) << pctProgress << " %" << COLOR_RESET
            << COLOR_LABEL << "    Restarts: "<< COLOR_VALUE << formatWithCommas(totalRestarts) << COLOR_RESET
            << COLOR_LABEL << "    Elapsed: "<< COLOR_VALUE << formatElapsedTime(elapsedTime) << COLOR_RESET;
        auto content = padAnsi(tmp.str(), inner);
        os << COLOR_FRAME << "|" << COLOR_RESET
           << content
           << COLOR_FRAME << "|" << COLOR_RESET << "\n";
...
int main(int argc, char* argv[])
{
    ...
    for(int i=1;i<argc;++i)
        if(!strcmp(argv[i],"-c")) askLoad=true;
    ...
    if(askLoad){
        if(loadConfig(cfg)) std::cout<<"Config loaded from "<<kConfigFile<<".\n";
        else std::cout<<"Error loading config.\n";
    }
    ...
    if(!cfg.loaded){
        ...
        if(!e.empty()&&(e[0]=='Y'||e[0]=='y')){
            ...
            if(p1!=p2){ std::cerr<<"Passphrase mismatch.\n"; return 1; }
            g_encryptResult = true;
            g_passphrase    = p1;
        }

        int defC = omp_get_num_procs();
        ...
        std::cout<<"Enter random hex digits for suffix (>0 required for Hybrid mode): ";
        std::getline(std::cin, th);
        cfg.randomHexCount = !th.empty()? std::stoi(th) : 0;
        if(cfg.randomHexCount<=0){
            std::cerr<<"Error: Hybrid mode requires a positive number of random hex digits.\n";
            return 1;
        }
    ...
    try {
        ...
        for(char c:cfg.address)
            sLower.push_back(std::tolower((unsigned char)c));
    ...
    if(pos==std::string::npos || cfg.range.substr(0,pos).empty() || cfg.range.substr(pos+1).empty()){
        std::cerr<<"Range is invalid or empty.\n";
        return 1;
    }

    std::string rangeStart = cfg.range.substr(0,pos);
    std::string rangeEnd   = cfg.range.substr(pos+1);
    size_t maxWidth = std::max(rangeStart.size(), rangeEnd.size());
    rangeStart = padHex(rangeStart, maxWidth);
    rangeEnd   = padHex(rangeEnd,   maxWidth);
    ...
    auto divRes = bigNumDivide(rangeSize, (uint64_t)numThreads);
    std::vector<uint64_t> chunkBNv = divRes.first;
    uint64_t remVal = divRes.second;

    struct TRange{ std::string startHex, endHex; };
    std::vector<TRange> thr(numThreads);
    auto curBNv = startBN;
    for(int i=0;i<numThreads;++i){
        auto cEnd = bigNumAdd(curBNv, chunkBNv);
        if((uint64_t)i < remVal)
            cEnd = bigNumAdd(cEnd, singleElementVector(1ULL));
        cEnd = bigNumSubtract(cEnd, singleElementVector(1ULL));
        thr[i].startHex = bigNumToHex(curBNv);
        thr[i].endHex   = bigNumToHex(cEnd);
        curBNv = bigNumAdd(cEnd, singleElementVector(1ULL));
    }
    std::string displayRange = thr.front().startHex + ":" + thr.back().endHex;

    __uint128_t suffixTotal128 = 1;
    long double totalCombosLD = 1.0L;
    for(int i=0;i<cfg.randomHexCount;++i){
        suffixTotal128 *= 16;
        totalCombosLD  *= 16.0L;
    }
    std::string totalCombosStr = formatWithCommas128(uint128ToString(suffixTotal128));
...
#pragma omp parallel num_threads(numThreads) shared(thr, targetHash)
    {
        ...
        uint64_t seqPrefix = 0, seqStartNum = 0, seqEndNum = 0;
        int seqDigits = 0;
        {
            int rangeW = (int)thr[tid].startHex.size();
            seqDigits  = rangeW - cfg.randomHexCount;
            if(seqDigits<1) seqDigits=1;
            std::string sp = thr[tid].startHex.substr(0, seqDigits);
            std::string ep = thr[tid].endHex.substr(0, seqDigits);
        ...
        std::vector<Point> plusP(POINTS_BATCH_SIZE), minusP(POINTS_BATCH_SIZE);
        ...
        alignas(32) static thread_local uint8_t pubKeys[512][33];
        alignas(32) static thread_local uint8_t hashOut[HASH_BATCH_SIZE][20];
...
if (seqDigits <= 16) {
    ...
    prefix = high + lowBuf;
            ...
            Int batchKey;
            try {
                Int tmp; tmp.SetBase16(const_cast<char*>(padded.c_str()));
                batchKey.Set(&tmp);
            } catch(...) {
                ++seqPrefix;
                continue;
            ...
            for(int i=0;i<POINTS_BATCH_SIZE;++i)
                deltaX[i].ModSub(&plusP[i].x, &startP.x);
            ...
            for(int i=0;i<POINTS_BATCH_SIZE;++i){
                Point t = startP;
                Int dY; dY.ModSub(&minusP[i].y, &startP.y);
                Int slope; slope.ModMulK1(&dY, &deltaX[i]);
                Int slopeSq; slopeSq.ModSquareK1(&slope);
                Int tmpX; tmpX.Set(&startP.x);
                tmpX.ModNeg(); tmpX.ModAdd(&slopeSq); tmpX.ModSub(&minusP[i].x);
                t.x.Set(&tmpX);
                Int dfX; dfX.Set(&startP.x);
                dfX.ModSub(&t.x); dfX.ModMulK1(&slope);
                t.y.ModNeg(); t.y.ModAdd(&dfX);
                pBatch[POINTS_BATCH_SIZE+i] = t;
            }
            ...
            for(int i=0;i<fullBatch;++i){
                pointToCompressedBin(pBatch[i], pubKeys[localBatch]);
                idxBuf[localBatch] = i;
                ++localBatch;

                if(localBatch == HASH_BATCH_SIZE){
                    computeHash160BatchBinSingle(localBatch, pubKeys, hashOut);
                    for(int j=0;j<localBatch;++j){
                        __m128i c16 = _mm_loadu_si128((const __m128i*)hashOut[j]);
                            ...
                            #pragma omp critical
                            {
                                if(!g_found.load()){
                                    ...
                                    if(idx < POINTS_BATCH_SIZE){
                                        ...
                                        Int off; off.SetInt32(idx - POINTS_BATCH_SIZE);
                                        finalPriv.Sub(&off);
                                    }
                                    ...
                                    sendMatchEmail(g_foundPriv, g_foundPub, g_foundWIF,
                                                   targetAddress, tot,
                                                   formatElapsedTime(dt), g_finalSpeed);
                                }
                            }
                            break;
                        }
